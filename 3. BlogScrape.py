...
spiders = []
output_file_name = 'BlogsLeftToRead2'
...
def main():
    ...
    custom_config_file = open(os.path.join(main_dir, 'config', 'singulartestconfig.yaml'), 'r', encoding='UTF-8')
    custom_config = yaml.load(custom_config_file)

    settings = get_project_settings()
    process = CrawlerProcess(settings)

    #clean_up_directory(main_dir + '/temp/blog_scrape_output')
    folder = main_dir + '/temp/blog_scrape_output'
